  ...
  const handleRowClick = async (player: Player) => {
    ...
    try {
      const res = await fetch(
        `https://api.kaiwaii4ever.win/api/user/player-data?UserID=${player.userId}&universeID=8244995861&datastore=PlayerStore`,
        { cache: "no-store" }
      );
      ...
      if (data) {
        ...
        const mappedData: PlayerDetails = {
          ...
          Data: [
            {
              universeId: "8244995861",
              success: firstItem.success ?? false,
              keys: firstItem.data?.keys,
              fullData: firstItem.data || {},
              error: firstItem.error || undefined,
            },
          ],
        };
  ...
  return (
    <PageContainer>
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
        ...
        <GradientText variant="h4">Player Store [YATB]</GradientText>
      </Box>
        ...
        <DialogContent dividers sx={{ maxHeight: "70vh", overflowY: "auto" }}>
          {loadingDetails ? (
            ...
            <DataBox>
              ...
              {playerDetails.Data[0].keys && (
                <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                  Keys: {playerDetails.Data[0].keys.join(", ")}
                </Typography>
              )}
              <pre style={{ margin: 0, fontSize: "0.875rem" }}>
                {JSON.stringify(playerDetails.Data[0].fullData, null, 2)}
              </pre>
            </DataBox>
          )}
        </DialogContent>
