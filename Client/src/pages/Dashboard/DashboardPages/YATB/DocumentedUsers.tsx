...
const DocumentedUsers: React.FC = () => {
  ...
  const handleRowClick = async (player: Player) => {
    try {
      const res = await fetch(
        `https://api.kaiwaii4ever.win/api/user/player-data?UserID=${player.userId}&universeID=8244995861`,
        { cache: 'no-store' }
      );
    ...
    <PageContainer>
      <HeaderBox>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
          ...
          <GradientText variant="h4">Documented Users [YATB]</GradientText>
        </Box>
...
export default DocumentedUsers;